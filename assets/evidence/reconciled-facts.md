...
3. **Gajae-Code / gjc**는 agent-run report에서는 public source 미확인으로 Low confidence 처리됐지만, controller cross-check에서 public repo와 npm package 및 README/docs를 확인했다. 후속 pass에서는 Bun/global package install 후 `gjc/0.7.3` basic smoke가 통과했고, isolated temp repo에서 controlled workflow artifact replay도 통과했다. 단, live team/recovery/Hermes bridge replay는 아직 미완료다.
...
| Gajae-Code | npm `gajae-code 0.7.3` and `@gajae-code/coding-agent 0.7.3`; local `gjc/0.7.3` version/help/smoke/print smoke passed; controlled temp-repo workflow artifact replay passed | Medium/High for basic runtime and artifact-backed workflow surface; live team/recovery/bridge pending |
