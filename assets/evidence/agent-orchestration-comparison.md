# Evidence: Agent Orchestration Comparison
...
| Current strong proof | source anchors for package/CLI/`.gjc`/workflow layout | local installed version proof plus source architecture map | plugin install/manifest proof plus `$ultraresearch` artifact journal |
| Current missing proof | local `gjc` smoke; public npm/GitHub provenance; Hermes bridge runtime | source/local version parity; gateway/cron/MCP/delegate runtime traces | isolated replay for non-ultraresearch `$...` workflows; MCP callability; TUI/exec parity |
...
| Best next probe | Bun-enabled `gjc --smoke-test` and npm/GitHub provenance check | installed/source version matrix and isolated read-only runtime probes | runtime-callable matrix with temp-repo workflow replays |
